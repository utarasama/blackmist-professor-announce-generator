    ...
    for course_line in range(4, 21, 4):
        if reduced_data.iloc[course_line, 1] != "COURS":
            courses += f"- {str(reduced_data.iloc[course_line, 0]).replace('\n', " ")}" + '\n' \
                    f'{reduced_data.iloc[course_line, 1]} | Pr. {reduced_data.iloc[course_line + 2, 1]}' + '\n' \
                    f'{reduced_data.iloc[course_line + 1, 1]} - {reduced_data.iloc[course_line + 3, 1]} | Obligatoire' + '\n\n'
    return courses
